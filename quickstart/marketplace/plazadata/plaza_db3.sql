...
UPDATE `oc_setting` SET `value` = 'Safira Marketplace 3 - Responsive Opencart Theme' WHERE `oc_setting`.`key` = 'config_meta_title' AND store_id = 0;
UPDATE `oc_setting` SET `value` = 'Safira Marketplace 3' WHERE `oc_setting`.`key` = 'config_owner' AND store_id = 0;
UPDATE `oc_setting` SET `value` = 'Safira Marketplace 3' WHERE `oc_setting`.`key` = 'config_name' AND store_id = 0;
UPDATE `oc_setting` SET `value` = 'tt_safira_marketplace3' WHERE `oc_setting`.`key` = 'theme_default_directory' AND store_id = 0;
